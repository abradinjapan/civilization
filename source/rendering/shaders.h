...
typedef struct SHADER__error_log {
    BASIC__u8 log[SHADER__opengl_error_log_length];
...
SHADER__error_log SHADER__create_null__error_log() {
    ...
    // setup output
    for (BASIC__u64 character_index = 0; character_index < SHADER__opengl_error_log_length; character_index += 1) {
        output.log[character_index] = 0;
    }
    output.opengl_error_code = GL_NO_ERROR;
...
typedef struct SHADER__shader {
    GLuint shader_ID;
    BASIC__buffer program;
} SHADER__shader;

typedef struct SHADER__program {
    GLuint program_ID;
    SHADER__shader vertex_shader;
    SHADER__shader fragment_shader;
} SHADER__program;
...
SHADER__shader SHADER__compile__shader(SHADER__et* error, SHADER__error_log* error_log, BASIC__buffer shader_data, GLenum shader_type) {
    ...
    // create shader space
    output.shader_ID = glCreateShader(shader_type);

    // send program to gpu
    glShaderSource(output.shader_ID, 1, (const GLchar* const*)&(shader_data.start), NULL);

    // compile shader
    glCompileShader(output.shader_ID);

    // check for errors
    glGetShaderiv(output.shader_ID, GL_INFO_LOG_LENGTH, &error_log_length);
    if (error_log_length > 0) {
        ...
        glGetShaderInfoLog(output.shader_ID, SHADER__opengl_error_log_length, NULL, (GLchar*)&((*error_log).log));
        (*error_log).opengl_error_code = glGetError();
    }
...
SHADER__program SHADER__create_null__shader_program() {
    ...
    // setup output
    output.program_ID = 0;
    output.vertex_shader = SHADER__create_null__shader();
    output.fragment_shader = SHADER__create_null__shader();
...
SHADER__program SHADER__compile__shaders_program(SHADER__et* error, SHADER__error_log* error_log, BASIC__buffer vertex_shader, BASIC__buffer fragment_shader) {
    ...
    *error = SHADER__et__no_error;
    ...
    // compile shaders
    output.vertex_shader = SHADER__compile__shader(error, error_log, vertex_shader, GL_VERTEX_SHADER);
    if (*error != SHADER__et__no_error) {
        return output;
    }

    output.fragment_shader = SHADER__compile__shader(error, error_log, fragment_shader, GL_FRAGMENT_SHADER);
    if (*error != SHADER__et__no_error) {
        return output;
    }

    // create shader program
    output.program_ID = glCreateProgram();

    // setup shader linking
    glAttachShader(output.program_ID, output.vertex_shader.shader_ID);
    glAttachShader(output.program_ID, output.fragment_shader.shader_ID);
    
    // link shaders
    glLinkProgram(output.program_ID);
    
    // check for errors
    glGetProgramiv(output.program_ID, GL_LINK_STATUS, &error_log_length);
    if (error_log_length > 1) {
        // get internal error type
        *error = SHADER__et__shaders_could_not_link;

        // get opengl error data
        glGetProgramInfoLog(output.program_ID, SHADER__opengl_error_log_length, NULL, (GLchar*)&((*error_log).log));
        (*error_log).opengl_error_code = glGetError();
    }
...
void SHADER__use__program(SHADER__program shaders_program) {
    glUseProgram(shaders_program.program_ID);
...
void SHADER__close__program(SHADER__program shaders_program) {
    glDeleteShader(shaders_program.vertex_shader.shader_ID);
    glDeleteShader(shaders_program.fragment_shader.shader_ID);
    glDeleteProgram(shaders_program.program_ID);
